...
// Footer End
	...
	.success(function (data) {
		...
		$.each(data.verson_items, function (index, d) {
			...
			news_lists += "<a href='" + d.dl_url + "'><img src='http://mtrsoftware.com.tw/images/download.gif'></a>";
			news_lists += "<div>更新日期<br>" + d.up_date + "</div>";
			news_lists += "</td></tr>";
			$('#verson_dl tbody').append(news_lists);
		});
		$.each(data.pharmacy_items, function (index, d) {
			pharmacy_lists = "<section class='media'><div class='media-left'>"
			pharmacy_lists += "<time><strong>" + d.dd + "</strong>"
			pharmacy_lists += "<b>" + d.mm + " 月</b>"
			pharmacy_lists += "<b>" + d.yyy + "年</b></time></div>"
			pharmacy_lists += "<div class='media-body'>"
			pharmacy_lists += "<h4 class='media-heading'>" + d.title + "</h4>"
			pharmacy_lists += "<ol>" + d.ol_cont + "</ol><div>"
			pharmacy_lists += "<p><i class='fa fa-bullseye'></i> DrugVer" + d.verson + "</p>"
			pharmacy_lists += "<span class='label label-default'><i class='fa fa-calendar'></i> 更新 " + d.update + "</span> "
			pharmacy_lists += "<a class='btn btn-xs btn-primary' href='" + d.dl_url + "'><i class='fa fa-download'></i> 開始下載</a>"
			pharmacy_lists += "</div></div></section>"
			$('#T_c').append(pharmacy_lists);
		});
	})
	.done(function () {
		$("#loading").hide();
